...
- `TRAINING/ranking/cross_sectional_feature_ranker.py` — Integrated config cleaner, added stability tracking
...
- **Files**:
  ...
  - `TRAINING/orchestration/training_plan_generator.py` - Converts routing → training jobs
...
- **Current Status**:
  - ✅ Cross-sectional training filtering: Fully implemented - only targets with CS jobs in plan are trained
  - ✅ Training plan generation: Fully implemented for both CS and symbol-specific routes
  - ⚠️ Symbol-specific training execution: Plan generated but per-symbol model execution requires future pipeline integration
- **Artifacts**:
  - `METRICS/training_plan/training_plan.json` - Machine-readable job specifications
...
**What Gets Trained**
- ✅ **Cross-sectional models**: Trained for targets with CS jobs in plan (pooled across all symbols)
- ⚠️ **Symbol-specific models**: Plan generated but execution not yet integrated (future enhancement)
...
- **One-Command Flow**: Complete end-to-end pipeline in single command:
  - Target ranking → Feature selection → Training plan generation → Training execution
  - Training plan automatically passed to training phase (auto-detected from common locations)
  - No manual intervention required
- **Auto-Detection**: Training phase automatically finds training plan in common locations (`results/METRICS/training_plan/`, etc.)
...
- **Documentation Reorganization**: Moved internal/planning docs to `INTERNAL_DOCS/training_routing/`, kept operational docs in `DOCS/02_reference/training_routing/`
...
- **Classification**: Three-tier system (STABLE/DRIFTING/DIVERGED) with stricter thresholds than per-symbol:
  - STABLE: overlap ≥0.75, tau ≥0.65 (vs. 0.7/0.6 for per-symbol)
...
- **Benefits**:
  ...
  - Enables feature routing (stable global → production, unstable → experimental)
...
- `TRAINING/utils/reproducibility_tracker.py` - Cohort-aware tracking, immediate writes, enhanced metadata, symbols list
- `TRAINING/utils/cohort_metadata_extractor.py` - **NEW** - Unified metadata extraction utility
- `TRAINING/ranking/predictability/model_evaluation.py` - Uses unified extractor, passes output_dir to LeakageAutoFixer
...
- `TRAINING/orchestration/intelligent_trainer.py` - RESULTS directory organization, cohort-based sorting, cache initialization fix
- `TRAINING/common/leakage_auto_fixer.py` - Integrated backups, output_dir support
- `CONFIG/training_config/safety_config.yaml` - Updated comments, cohort_aware default

### Documentation Updated

- `DOCS/03_technical/implementation/COHORT_AWARE_REPRODUCIBILITY.md` - Updated storage structure, added symbols field
- `DOCS/03_technical/implementation/REPRODUCIBILITY_STRUCTURE.md` - Updated with RESULTS directory structure
- `DOCS/01_tutorials/training/INTELLIGENT_TRAINING_TUTORIAL.md` - Updated output structure section
