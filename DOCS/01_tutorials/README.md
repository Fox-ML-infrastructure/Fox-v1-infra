# Tutorials

Step-by-step guides and walkthroughs for common tasks and workflows.

## Purpose

This directory contains practical, hands-on tutorials that guide users through specific tasks. Each tutorial focuses on a single workflow or feature with clear examples and expected outcomes.

## Contents

### Setup
- **[Installation](setup/INSTALLATION.md)** - System installation guide
- **[Environment Setup](setup/ENVIRONMENT_SETUP.md)** - Python environment configuration
- **[GPU Setup](setup/GPU_SETUP.md)** - GPU configuration for training and ranking

### Configuration
- **[Config Basics](configuration/CONFIG_BASICS.md)** - Configuration fundamentals
- **[Experiment Config Guide](configuration/EXPERIMENT_CONFIG_GUIDE.md)** - Experiment configuration files
- **[Config Examples](configuration/CONFIG_EXAMPLES.md)** - Example configurations
- **[Advanced Config](configuration/ADVANCED_CONFIG.md)** - Advanced configuration techniques

### Training
- **[Intelligent Training Tutorial](training/INTELLIGENT_TRAINING_TUTORIAL.md)** - Automated pipeline workflow
- **[Auto Target Ranking](training/AUTO_TARGET_RANKING.md)** - Auto-discover and rank targets
- **[Model Training Guide](training/MODEL_TRAINING_GUIDE.md)** - Manual training workflow
- **[Feature Selection Tutorial](training/FEATURE_SELECTION_TUTORIAL.md)** - Manual feature selection
- **[Walk-Forward Validation](training/WALKFORWARD_VALIDATION.md)** - Validation workflow
- **[E2E Test Command](training/E2E_TEST_COMMAND.md)** - End-to-end testing guide
- **[Testing Quick Reference](training/TESTING_QUICK_REFERENCE.md)** - Quick testing reference

### Pipelines
- **[First Pipeline Run](pipelines/FIRST_PIPELINE_RUN.md)** - Run your first pipeline
- **[Data Processing Walkthrough](pipelines/DATA_PROCESSING_WALKTHROUGH.md)** - Data pipeline guide
- **[Feature Engineering Tutorial](pipelines/FEATURE_ENGINEERING_TUTORIAL.md)** - Feature creation

### General
- **[Refactoring & Wrappers](REFACTORING_AND_WRAPPERS.md)** - Understanding modular structure
- **[Simple Pipeline Usage](SIMPLE_PIPELINE_USAGE.md)** - Simplified pipeline usage

## Who Should Read This

- **New Users** - Start with Setup, then Configuration, then Training
- **Quants/Researchers** - Training tutorials, Pipeline tutorials
- **ML Engineers** - All tutorials for comprehensive understanding

## Related Documentation

- [Executive Documentation](../00_executive/) - High-level overviews
- [Reference Documentation](../02_reference/) - Technical reference
- [Technical Documentation](../03_technical/) - Deep technical details

